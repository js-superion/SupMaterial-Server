...
public class BillNoGenerator {
	 ...
	 * @throws Exception
	 */
	public static String getNextNo(String fstrRcptType, String fstrCurTypeDate,
			String fstrCurNo, String fstrNowTypeDate) throws Exception {
		// YYYYMMDD+4位数字
		...
		return parseNo(fstrCurTypeDate, fstrCurNo, fstrNowTypeDate, 4);
		// throw new
		// RuntimeException("流水号类型值["+fstrRcptType+"]不正确,应在[1,2,3,4,5,6]集合中取值");
	}

	public static String getNowTypeDate(String fstrRcptType) {
		if ("1".equals(fstrRcptType) || "2".equals(fstrRcptType)
				|| "3".equals(fstrRcptType)) {
			return DateUtil.getCurrentDateVersion();
		} else if ("4".equals(fstrRcptType) || "5".equals(fstrRcptType)
				|| "6".equals(fstrRcptType)) {
			return DateUtil.getCurYYMM();
		}
		throw new RuntimeException("流水号类型值[" + fstrRcptType
				+ "]不正确,应在[1,2,3,4,5,6]集合中取值");
	}

	private static String parseNo(String fstrCurTypeDate, String fstrCurNo,
			String fstrNowTypeDate, int digital) {
	...
	public static void main(String[] args) {
		try {
			System.out.println(getNextNo("3", "20101111", "04",
					getNowTypeDate("3")));
			System.out.println(getNextNo("2", null, null, getNowTypeDate("2")));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
